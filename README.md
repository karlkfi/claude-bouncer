...
| <img src="plugins/exit-status-guard/docs/img/favicon-48.png" width="24" height="24" alt=""> | [exit-status-guard](plugins/exit-status-guard) | 2.0.0 | A gate whose failure reads as success: piped into a filter, backgrounded behind an `echo`, or sequenced before a state change with `;` — plus the zsh-only expansions that read as success on their own |
