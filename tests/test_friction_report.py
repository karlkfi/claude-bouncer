...
# An overridden call leaves no decision to read: the hook defers on a
# FOREGROUND_GUARD_OVERRIDE prefix, so the count comes off the command string.
CMD_OVERRIDE = "FOREGROUND_GUARD_OVERRIDE=demo gh run watch 456"
# A sibling guard's override on a call this guard never spoke about. In scope
# under --plugin all, and never a foreground-guard override.
CMD_SIBLING_OVERRIDE = "PROD_GUARD_OVERRIDE=x kubectl delete ns x"
REASON_SIBLING = "prod-guard: matches a production pattern"
...
REASON_DENY = REASON_LOOP + " " + guard.deny_tail()
...
class CategoryTests(unittest.TestCase):
    ...
    def test_override_signature_reads_a_command_not_a_reason(self):
        # The guard defers on an override, so there is no reason text to key
        # on — the signature has to match the prefix on the command itself.
        self.assertTrue(fr.OVERRIDE_SIG.search(CMD_OVERRIDE))
        self.assertFalse(fr.OVERRIDE_SIG.search(CMD_SIBLING_OVERRIDE))
        self.assertFalse(fr.OVERRIDE_SIG.search("gh run watch 456"))
...
class DenyFromResultTests(unittest.TestCase):
    ...
    def test_deny_tail_does_not_invent_a_target(self):
        watch_deny = REASON_WATCH + " " + guard.deny_tail()
...
class BuildReportTests(unittest.TestCase):
    ...
    def test_override_count_comes_from_the_tally(self):
        # An overridden call produces no decision, so build_report cannot see
        # it: the number is carried in from the command walk.
        r = fr.build_report(
            [{"plugin": "foreground-guard", "decision": "ask",
              "reason": REASON_WATCH, "command": "gh run watch 456"}],
            {"bash_calls": 9, "sessions": 1, "sessions_prompted": 1,
             "overrides": 2})
        self.assertEqual(r["overrides"], 2)

    def test_override_count_is_zero_without_a_tally(self):
        # No command walk, no override count — reported as zero rather than
        # guessed at from the decisions.
        self.assertEqual(self._report([])["overrides"], 0)
...
class PrintTests(unittest.TestCase):
    ...
    def _override_report(self):
        return fr.build_report(
            [{"plugin": "foreground-guard", "decision": "ask",
              "reason": REASON_WATCH, "command": "gh run watch 456"}],
            {"bash_calls": 4, "sessions": 1, "sessions_prompted": 1,
             "overrides": 1})

    def test_override_line_shown_for_this_guard(self):
        buf = io.StringIO()
        with redirect_stdout(buf):
            fr.print_text(self._override_report(), 15)
        self.assertIn("FOREGROUND_GUARD_OVERRIDE prefixes (guard deferred): 1",
                      buf.getvalue())

    def test_override_line_omitted_under_plugin_all(self):
        buf = io.StringIO()
        with redirect_stdout(buf):
            fr.print_text(self._override_report(), 15, "all")
        self.assertNotIn("FOREGROUND_GUARD_OVERRIDE", buf.getvalue())

    def test_override_line_omitted_under_a_sibling_plugin(self):
        # The count now comes off the commands, so it no longer depends on
        # which guard spoke — the report has to withhold it itself rather
        # than print this guard's statistic under prod-guard's header.
        buf = io.StringIO()
        with redirect_stdout(buf):
            fr.print_text(self._override_report(), 15, "prod-guard")
        self.assertNotIn("FOREGROUND_GUARD_OVERRIDE", buf.getvalue())
...
class EndToEndTests(unittest.TestCase):
    ...
    def test_override_counted_from_a_call_with_no_decision(self):
        # The whole point of reading the command: the guard deferred, so this
        # call has a tool_use record and nothing else. Counting the decision
        # stream would report zero.
        use1, att1 = _decision_record(
            "toolu_1", "gh run watch 456", _stdout("deny", REASON_DENY))
        root = write_transcript([use1, att1, _bash_call("toolu_2", CMD_OVERRIDE)])
        data = json.loads(self._run(root, "--json"))
        self.assertEqual(data["overrides"], 1)
        self.assertEqual(data["bash_calls"], 2)
        self.assertEqual(data["total"], 1)
        self.assertIn("FOREGROUND_GUARD_OVERRIDE prefixes (guard deferred): 1",
                      self._run(root))
    ...
    def test_sibling_override_never_reported_as_ours(self):
        use1, att1 = _decision_record(
            "toolu_1", "gh run watch 456", _stdout("ask", REASON_WATCH))
        use2, att2 = _decision_record(
            "toolu_2", CMD_SIBLING_OVERRIDE, _stdout("ask", REASON_SIBLING),
            hook_cmd='python3 "/x/bash-prod-guard.py"')
        root = write_transcript([use1, att1, use2, att2])
