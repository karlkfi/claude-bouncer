...
|---|---|---|---|---|---|
| <a id="Q14"></a>Q14 | Add CI workflow to run the test suite on PRs | `infra` `tests` | 🔲 | S | GitHub Actions workflow running `python3 -m unittest discover tests` on push and pull requests. Unblocks [Q13](#Q13) and gates the security-fix chain below. |
...
| <a id="Q13"></a>Q13 | Add CI status badge to README once CI exists | `docs` | 🚫 | S | Blocked by [Q14](#Q14). Once a CI workflow runs the suite, add a workflow-run badge to the README badge row. A green CI badge with no CI is worse than no badge. |
