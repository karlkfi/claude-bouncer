...
|---|---|---|---|---|---|
| <a id="Q25"></a>Q25 | Expose friction-report as a `/friction-report` slash command | `docs` `infra` | 🔲 | S | Thin `commands/friction-report.md` wrapper exec'ing `${CLAUDE_PLUGIN_ROOT}/scripts/friction-report.py` with `$ARGUMENTS` passthrough, for the "just show me the data" intent the [Q24](#Q24) skill doesn't cover. Verify `${CLAUDE_PLUGIN_ROOT}` expands in a command's `!`-bash before shipping; update README + plugin keywords. |
