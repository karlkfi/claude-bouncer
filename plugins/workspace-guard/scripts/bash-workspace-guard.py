...
# copy under this plugin's `lib/` is vendored from the repository root; see
...
def poison_vars(g, varmap):
    ...
    for t in kw_g[:len(kw_g) - len(rest)]:
        # `strip_env_prefix` peels `NAME+=v` too, so the name has to come off
        # the same way it does everywhere else -- splitting on `=` would poison
        # `NAME+` and leave `NAME` on the map at its stale value (Q174).
        varmap.pop(split_assignment(t)[0], None)
    if rest:
