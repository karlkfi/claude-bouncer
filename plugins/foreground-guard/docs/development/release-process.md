...
- `plugins/foreground-guard/.claude-plugin/plugin.json` → `"version"`
...
The marketplace entry is the one `claude plugin update` compares, so a bump that misses it ships nothing while the README announces a new version. A one-sided bump of the two JSON files also fails `WiringTests.test_plugin_and_marketplace_agree` in `tests/test_foreground_guard.py`. To read all three, from the repository root:
...
3. **Bump all three version strings** to the new `X.Y.Z`. Patch (`Z`) for fixes, docs, and packaging; minor (`Y`) for a new guarded form (a `BUILTIN_WATCH` row, a Class B registry addition) or a change to the hook surface; major (`X`) for a default-behavior change (e.g. a class defaulting to `deny`). Most releases are patch.
